...
//            case TOUCH_MODE_OVERSCROLL:
    ...
    private void scrollIfNeeded(final int y) {
        if (DBG) Log.i(TAG, "scrollIfNeeded y: " + y);
        final int rawDeltaY = y - mMotionY;
        final int deltaY = rawDeltaY - mMotionCorrection;
        int incrementalDeltaY = mLastY != Integer.MIN_VALUE ? y - mLastY : deltaY;

        if (mTouchMode == TOUCH_MODE_SCROLLING) {
            if (DBG) Log.i(TAG, "scrollIfNeeded TOUCH_MODE_SCROLLING");
            if (y != mLastY) {
                // stop our parent
    ...
    private boolean moveTheChildren(int deltaY, int incrementalDeltaY) {
        if (DBG) Log.i(TAG, "moveTheChildren deltaY: " + deltaY + "incrementalDeltaY: " + incrementalDeltaY);
        ...
        if (cannotScrollDown) {
            if (DBG) Log.i(TAG, "moveTheChildren cannotScrollDown " + cannotScrollDown);
            return incrementalDeltaY != 0;
        }

        if (cannotScrollUp) {
            if (DBG) Log.i(TAG, "moveTheChildren cannotScrollUp " + cannotScrollUp);
            return incrementalDeltaY != 0;
        }
        ...
        if (count > 0) {
            if (DBG) Log.i(TAG, "scrap - detachViewsFromParent start:" + start + " count:" + count);
    ...
    private View fillDown(int pos, int nextTop) {
        if (DBG) Log.i(TAG, "fillDown - pos:" + pos + " nextTop:" + nextTop);
    ...
    private View fillUp(int pos, int nextBottom) {
        if (DBG) Log.i(TAG, "fillUp - position:" + pos + " nextBottom:" + nextBottom);
        View selectedView = null;
        ...
        while ((nextBottom > end || hasSpaceUp()) && pos >= 0) {
            ...
            if (DBG) Log.i(TAG, "fillUp next - position:" + pos + " nextBottom:" + nextBottom);
        }
    ...
    private void setupChild(View child, int position, int y, boolean flowDown,
        ...
        if (recycled || (layoutParams.recycledHeaderFooter &&
            ...
            if (DBG) Log.i(TAG, "setupChild attachViewToParent position:" + position);
            attachViewToParent(child, flowDown ? -1 : 0, layoutParams);
        }
        else {
            if (DBG) Log.i(TAG, "setupChild addViewInLayout position:" + position);
        ...
        if (needToMeasure) {
            if (DBG) Log.i(TAG, "setupChild onMeasureChild position:" + position);
            onMeasureChild(child, layoutParams);
        }
        else {
            if (DBG) Log.i(TAG, "setupChild cleanupLayoutState position:" + position);
    ...
    private View obtainView(int position, boolean[] isScrap) {
        ...
        View child;
        if (scrapView != null) {
            if (DBG) Log.i(TAG, "getView from scrap position:" + position);
            ...
            if (DBG) Log.i(TAG, "getView position:" + position);
    ...
    protected void offsetChildrenTopAndBottom(int offset) {
        if (DBG) Log.i(TAG, "offsetChildrenTopAndBottom: " + offset);
        final int count = getChildCount();
    ...
    class RecycleBin {
        ...
        void addScrapView(View scrap, int position) {
            if (DBG) Log.i(TAG, "addScrapView position = " + position);

...
public class StaggeredGridView extends ExtendableListView {
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        Log.e("StaggeredGridView", "onMeasure called,mColumnWidth = " + mColumnWidth);
    }
    ...
    protected void onMeasureChild(final View child, final LayoutParams layoutParams) {
        ...
        if (viewType == ITEM_VIEW_TYPE_HEADER_OR_FOOTER ||
            ...
            if (DBG) Log.i(TAG, "onMeasureChild BEFORE position:" + position +
                    " h:" + getMeasuredHeight());
        ...
        if (DBG) Log.i(TAG, "onMeasureChild AFTER position:" + position +
                " h:" + childHeight);
    }
    ...
    protected void onChildCreated(final int position, final boolean flowDown) {
        ...
        if (!isHeaderOrFooter(position)) {
            ...
            if (DBG) Log.i(TAG, "onChildCreated position:" + position +
                                " is in column:" + column);
        }
        else {
    ...
    private void layoutGridChild(final View child, final int position,
        ...
        if (DBG) Log.i(TAG, "onLayoutChild position:" + position +
                " column:" + column +
    ...
    private void offsetGridChild(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childTop) {
        ...
        if (DBG) Log.i(TAG, "onOffsetChild position:" + position +
                " column:" + column +
                " childTop:" + childTop +
                " gridChildTop:" + gridChildTop +
                " gridChildBottom:" + gridChildBottom);
    ...
    protected void offsetChildrenTopAndBottom(final int offset, final int column) {
        if (DBG) Log.i(TAG, "offsetChildrenTopAndBottom: " + offset + " column:" + column);
        final int count = getChildCount();
        for (int i = 0; i < count; i++) {
            final View v = getChildAt(i);
            if (v != null &&
    ...
    private void offsetDistanceToTop(final int offset) {
        ...
        if (DBG) Log.i(TAG, "offset mDistanceToTop:" + mDistanceToTop);
    }
    ...
    private void onColumnSync() {
        ...
        for (int pos = 0; pos < syncPosition; pos++) {
            ...
            Log.i(TAG, "onColumnSync:" + pos + " ratio:" + rec.heightRatio);
        ...
        if (DBG) Log.i(TAG, "onColumnSync column width:" + mColumnWidth);
        ...
        for (int pos = 0; pos < syncPosition; pos++) {
            ...
            if (DBG) Log.i(TAG, "onColumnSync position:" + pos +
                                " top:" + top +
                                " bottom:" + bottom +
                                " height:" + height +
                                " heightRatio:" + heightRatio);
    ...
    private void setPositionHeightRatio(final int position, final int height) {
        ...
        if (DBG) Log.i(TAG, "position:" + position +
                            " width:" + mColumnWidth +
                            " height:" + height +

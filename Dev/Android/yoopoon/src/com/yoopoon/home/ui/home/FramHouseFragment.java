...
public class FramHouseFragment extends FramSuper implements OnClickListener {
	...
	// 楼盘顶端条件绑定的Adapter
	FramHouseConditionAdapter mHouseConditionAdapter;
	// 首页广告
		...
		new RequestAdapter() {
			...
			public void onReponse(ResponseData data) {
				if (data.getResultState() == ResultState.eSuccess) {
					...
					initHouseTypeList(houseTypeJsonObjects, type_textview);
				}
			}
			@Override
			public void onProgress(ProgressMessage msg) {
			}
		}.setUrl(getString(R.string.url_house_condition)).setRequestMethod(RequestMethod.eGet).addParam(parameter)
				.notifyRequest();
	}
	/**
	 * 初始化首页顶端楼盘类型
	 ...
	 */
	private void initHouseTypeList(ArrayList<JSONObject> houseTypeJsonArray, View type_textview) {
